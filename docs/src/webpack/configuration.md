...
webapck是一个js应用程序的模块打包工具，我们将这句话拆解出来：

* 打包：本质上是将指定文件(`entry`)按一定的规则输出(`output`)为目标文件。
* webpack 只能理解 JavaScript 和 JSON 文件。所以，需要 `loader` 让 webpack 能够去处理其他类型的文件，并将它们转换为有效 [模块](https://v4.webpack.docschina.org/concepts/modules)。
